...
def run_analysis(input_path: Path, output_dir: Path, seed: int = 2026) -> None:
    ...
    scenarios = pd.DataFrame([
        {"scn":"Observed outcomes only","outcome_rate":data["outcome"].mean(),"mean_cost":data["cost"].mean(),"rows":data["outcome"].notna().sum()},
        {"scn":"Complete case","outcome_rate":complete["outcome"].mean(),"mean_cost":complete["cost"].mean(),"rows":len(complete)},
        {"scn":"Mean imputation for predictors","outcome_rate":mean_imp["outcome"].mean(),"mean_cost":mean_imp["cost"].mean(),"rows":mean_imp["outcome"].notna().sum()},
        {"scn":"Median imputation for predictors","outcome_rate":median_imp["outcome"].mean(),"mean_cost":median_imp["cost"].mean(),"rows":median_imp["outcome"].notna().sum()},
        {"scn":"All missing outcomes are 0","outcome_rate":worst,"mean_cost":data["cost"].mean(),"rows":total},
        {"scn":"All missing outcomes are 1","outcome_rate":best,"mean_cost":data["cost"].mean(),"rows":total},
    ])
    missing.to_csv(out/"missingness_profile.csv", index=False)
    scenarios.to_csv(out/"missing_data_scenarios.csv", index=False)

    fig, ax = plt.subplots(figsize=(8.5,5))
    ax.barh(scenarios["scn"][::-1], scenarios["outcome_rate"][::-1])
