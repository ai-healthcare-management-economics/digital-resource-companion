...
def run_analysis(input_path: Path, output_dir: Path, seed: int = 2026) -> dict:
    ...
    require_columns(scenarios,["scn","arrival_rate_per_hour","service_rate_per_hour","servers","priority_accuracy","operating_hours"],"Scenarios")
    ...
    for i,r in scenarios.iterrows():
        for rep in range(reps):
            ...
            res.update({"scn":r["scn"],"replication":rep,
                        "wait_cost":res["mean_wait_minutes"]/60*delay_cost*res["arrivals"]})
            rows.append(res)
    detail=pd.DataFrame(rows)
    summary=detail.groupby("scn",as_index=False).agg(
    ...
    fig,ax=plt.subplots(figsize=(8,5));ax.bar(summary["scn"],summary["mean_wait"])
    ...
    payload={"replications":reps,"best_scenario_by_wait":str(best["scn"]),
