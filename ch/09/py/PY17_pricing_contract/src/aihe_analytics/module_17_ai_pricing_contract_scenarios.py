...
def run_analysis(input_path: Path, output_dir: Path, seed: int = 2026) -> dict:
    ...
    require_columns(s,[
        "scn","fixed_fee","per_use_fee","eligible_volume","adoption_rate",
    ...
    ax.bar(r["scn"],r["net_benefit"])
    ax.axhline(0,linewidth=1)
    ax.set_ylabel("Net benefit");ax.set_title("AI contract scn comparison")
    ax.tick_params(axis="x",rotation=35)
    save_figure(fig,out/"pricing_scenarios.png")
    best=r.iloc[0]
    payload={"best_scenario":str(best["scn"]),"best_net_benefit":float(best["net_benefit"]),
